...
# Chargement de kableExtra avec gestion des warnings
suppressWarnings({
  library(kableExtra)
})
...
```{r qc_plots, fig.height=5, fig.cap="Distribution des métriques de contrôle qualité. De gauche à droite : nombre de gènes détectés par cellule (nFeature_RNA), nombre total d'UMIs par cellule (nCount_RNA), et proportion de gènes mitochondriaux."}
...
```{r elbow_plots, fig.height=4, fig.width=10, fig.cap="Détermination du nombre optimal de composantes principales. (A) Elbow plot standard montrant la variance expliquée par chaque PC. (B) Elbow plot avec ligne de connexion montrant la proportion de variance expliquée."}
...
cluster0_top <- markers %>% 
  ...
  dplyr::select(gene, avg_log2FC, pct.1)
